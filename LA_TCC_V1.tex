...
\usepackage{footnote}
\usepackage{geometry}
\geometry{a4paper, left=2cm, right=2cm, bottom=2cm, top=2cm, footskip=2cm}
% ---
...
%	\addcontentsline{toc}{chapter}{Introdução}
	...
	\par Nos transportes aéreos, o mesmo transponder passou a ser utilizado na aviação comercial, tornando-se item obrigatório. Este coopera para a comunicação de aviões com torres de comando além de ser parte importante dos sistemas anticolisão. Através da troca de informação entre estes sistemas, é possível que aviões projetem informações de outras aeronaves em um raio considerável, como localização, rumo, altitude e velocidade. O site \href{https://www.flightradar24.com}{flightradar24.com} apresenta dados públicos obtidos de aviões por meio de seus transponders conforme pode ser visto na \autoref{fig_0}.

	\begin{figure} [H]
		\centering
		\includegraphics[scale=.35]{figuras/cap1/01FlightRadar}
		\caption{\label{fig_0}Dados públicos de vôos}
	\end{figure}
	...
	\par Segundo expectativas de mercado, 75\% dos carros do mundo estarão conectados em 2020, e o crescimento da Internet dos Veículos trará em torno de US\$2,94 bilhões em receitas{\footnote{\href{http://press.trendforce.com/node/view/1880.html}{http://press.trendforce.com/node/view/1880.html} (acesso em 13 de maio 2016)}}. Com um amplo apoio do Departamento de Transporte dos Estados Unidos, a comunicação entre veículos está perto de se tornar obrigatória para automóveis em circulação neste país{\footnote{\href{https://www.transportation.gov/fastlane/dialogue-industry-conversation-between-cars}{https://www.transportation.gov/fastlane/dialogue-industry-conversation-between-cars} (acesso em 14 de maio 2016)}}. Na Europa, a implantação da comunicação veicular também está acelerada. Foi criado um consórcio chamado Car2Car{\footnote{\href{https://www.car-2-car.org/}{https://www.car-2-car.org/} (acesso em 22 de maio 2016)}}  a partir da iniciativa de fabricantes de veículos europeus. Este consórcio sem fins lucrativos tem por objetivo aumentar a eficiência e segurança no tráfego rodoviário a partir da utilização de redes veículares.
	...
	\par Como base teória para um estudo de caso, serão discorridos conceitos de redes de computadores e de redes veiculares, que foram padronizados pelo IEEE e outras organizações por meio das normas IEEE 802.11p, IEEE 1609 e SAE J2735. 
	...
	\par Cada camada OSI contém um conjunto de funções executadas por programas para fazer os dados viajarem de uma origem até o destino de uma rede. Tanenbaum e Wetherall (2011) discorrem sobre as camadas de maneira detalhada:
	...
	\par Forouzan (2007) desdobra a pilha de protocolos TCP/IP em quatro camadas: Aplicação, Transporte, Internet e Acesso à Rede. Apesar de algumas camadas do modelo TCP/IP serem homônimas de algumas do modelo OSI, estas têm diferentes funções em cada modelo. A \autoref{fig_2} compara as camadas do modelo OSI e TCP/IP.
	...
	\par O conceito de encapsulamento é tratado por Forouzan (2007) e descreve o processo de trâmite da informação em uma rede iniciando em uma origem e enviada a um destino. Esta seção explica como o processo de transmissão de dados funciona em redes. 
	...
	\par Existem três diferentes formas de compartilhamento de mensagens, chamados de \textit{Unicast}, \textit{Multicast} e Broadcast. Forouzan (2007) e Tanenbaum e Wetherall (2011) descrevem técnicas específicas para o envio da informação de acordo com cada possível cenário.
	...
	\par Tanenbaum e Wetherall (2011) agrupam as redes de computadores por categorias que as diferenciam de acordo com seu tamanho e área de cobertura. Existem as redes locais (LAN ou \textit{Local Area Networks}) e as redes de longo alcance (WAN ou \textit{Wide Area Networks}). As redes metropolitanas (MAN ou \textit{Metropolitan Area Networks}) se encaixam como categoria entre as LANs e WANs.
	...
	\par As LANs atuais são redes de alta velocidade (entre 100 Mbps a 10 Gbps), baixo índice de erros e cobrem uma área geográfica relativamente pequena (Tanenbaum e Wetherall, 2011). As LANs conectam estações de trabalho, periféricos, roteadores, \textit{switches} e outros dispositivos em um único prédio ou área geográfica. O padrão \textit{Ethernet} é de longe o tipo mais comum de LAN cabeada.
	...
	\par O IEEE por meio do padrão IEEE 802 subdividiu a camada de enlace em duas subcamadas nomeadas de LLC (\textit{Logical Link Control} ou Controle de Enlace Lógico) e MAC (\textit{Media Access Control} ou Controle de Acesso ao Meio).
	...
	\par Segundo Gast (2005), as redes sem fio suportam basicamente dois modos de operação: Infra-estruturado e ad hoc.
	...
	\begin{figure} [H]
		\centering
		\includegraphics[scale=.55]{figuras/cap2/08ModoInfra-estruturado}
	...
	\begin{figure} [H]
		\centering
		\includegraphics[scale=.45]{figuras/cap2/09ModosdeOperacaoAdHoc}
		\caption{\label{fig_9}Modo de Operação Ad-Hoc}
	\end{figure}
	
	\subsection{Arquitetura IEEE 802.11}
	
	\par Gast (2005) reporta que quando duas ou mais estações se comunicam umas com as outras, estas compartilham um Conjunto Básico de Serviços (BSS ou \textit{Basic Service Set}). Um BSS mínimo é formado entre dois nós. As redes locais 802.11 utilizam o BSS como um bloco de construção padrão.
	
	\par Gast (2005) ainda informa que o BSS que não se conecta a um AP é chamado de BSS Independente (\textit{Independent} BSS ou IBSS). Este tipo de BSS é comumente visto em Redes \textit{Ad Hoc}. A formação de um IBSS pode ser melhor entendida na \autoref{fig_10}.
	...
	\par O IEEE define diversos serviços que precisam ser fornecidos pela LAN para prover funcionalidade equivalente às LANs cabeadas. Gast (2005) cita os serviços mais importantes, que são:
	...
	\par As redes \textit{ad hoc} se destacam por serem redes onde não há um nó central - como um roteador ou um \textit{switch} - servindo como ponto de acesso. Esta função é acumulada pelos nós, que se conectam, consomem e geram dados, assim como armazenam e entregam informações. Em redes \textit{ad hoc} são trocados quadros de arquitetura IBSS, se diferenciando das redes de infraestrutura, que utilizam quadros BSS. Além disso, as redes \textit{ad hoc} não possuem conexão direta com a Internet. Justamente por serem redes sem infraestrutura, as mesmas não possuem provedor de acesso.
	...
	\par Um dispositivo Car2X é um meio de comunicação de rádio que pretende oferecer serviços interoperáveis para os transportes. Funciona como um “roteador” para veículos. Pode apenas transmitir ou receber de dados ou realizar ambas as operações.
	
	\par Atualmente, várias empresas produzem hardware, \textit{firmware} e software para dispositivos de tecnologia Car2X. Entre elas, se destacam a Cohda Wireless, Autotalks, Commsignia, Rohde e Schwarz, Vector Informatik e Unex.
	...
	\par Segundo o padrão IEEE 1609.0 (2013), os dispositivos Car2X têm duas principais formas de comunicação: veículo para veículo e veículo para infraestrutura.
	
	\par As comunicações de \textbf{veículo para veículo} (\textit{Vehicle to Vehicle} ou V2V) são transferências de informação apenas entre unidades de bordos de veículos. O protocolo WSMP é o que provê conexões para este tipo de comunicação no padrão WAVE.
	...
	\par As entidades WME e MLMEX (MLME \textit{Extension} ou Extensão MLME) herdam aspectos da SME (\textit{Station Management Entity} ou Entidade de Gerenciamento de Estação), estabelecida no padrão IEEE 802.11 (2012).
	...
	\par O escopo da camada de segurança em dispositivos WAVE é discutido no padrão IEEE 1609.2 (2016) sobre Serviços de Segurança para Aplicações e Mensagens de Gerenciamento, que tem os propósitos de estabelecer a privacidade nas comunicações WAVE e proteger os dispositivos WAVE de ataques e de vazamento de informações.
	...
	\par A camada física do modelo WAVE foi descrita no padrão IEEE 802.11p (2010), que é uma emenda à família de padrões 802.11. Esta emenda propõe uma adaptação das redes sem fio em suas camadas física e de enlace (subcamada MAC) para o ambiente veicular.

	\par Os meios sem fio de transmissão de dados mais utilizados, segundo o padrão IEEE 802.11 (2012), baseiam-se em Rádio Frequência (RF).  Existem outros meios de propagação sem fio como o Infravermelho, porém são menos utilizados. As frequências de rádio empregadas para redes de comunicação sem fio estão na faixa de frequência de micro-ondas no espectro eletromagnético (\autoref{fig_20}), variando entre 0,3x$10^{9}$ até 3x$10^{11}$ Hertz.
	...
	\par O espectro que segue na \autoref{fig_21}, descrito no padrão IEEE 1609.0 (2013), foi alocado pela Comissão Federal das Comunicações (FCC) dos Estados Unidos para o DSRC. É esperado que os sistemas WAVE desenvolvidos nos EUA trabalhem nos canais definidos pela FCC.
    ...
    \par Estes serviços “não anunciados” e “anunciados” serão aprofundados na \autoref{sssec:wsm} (Mensagem Curta WAVE) e na \autoref{sssec:wsa} (Anúncio de Serviços WAVE), respectivamente.

	\section{Subcamada MAC}
	
	\par Na camada de enlace também foram acrescidas emendas, sobretudo na subcamada MAC. Diferentemente de uma conexão sem fio tradicional, na qual o MAC do \textit{Access Point} está presente no cabeçalho do \textit{frame}, a norma IEEE 802.11p (2010) define um método de troca de informações mais rápido, sem o estabelecimento de um BSSID real. Um BSSID “coringa” (com os valores binários em 1’s) é utilizado no cabeçalho dos \textit{frames} trocados. Este é inserido no campo Endereço 3 do cabeçalho MAC (\autoref{fig_23}). 
	...
	\par Além disso, o padrão IEEE 802.11 (2012) adiciona à MIB (Ver mais em \autoref{MIB}) do protocolo de gerenciamento SNMP a variável \textbf{dot11OCBActivated} como booleana. OCB (\textit{On the Context of} BSS) significa no contexto do BSS. Caso for configurada como verdadeira e o BSSID Coringa for utilizado, a conexão entre os pares é realizada dispensando os procedimentos de associação, autenticação e confidencialidade de dados. Estes serviços então seriam de responsabilidade das camadas superiores.
	...
	\par Os dispositivos WAVE implementam operações e serviços que suportam conectividade multi-canal na subcamada de acesso ao meio (MAC), baseando-se na norma IEEE 1609.4 (2016). Esta norma descreve mecanismos efetivos que controlam a transferências de dados de camadas superiores através de múltiplos canais. Outra norma descrita nesta seção é a IEEE 802.11 MAC, que trata de forma mais abrangente a coordenação de múltiplos canais em redes sem fio.
	...
	\begin{itemize}
        ...
        \item Priorização de pacotes
    \end{itemize}
	
	\par O \textbf{serviço de coordenação de canal} oferecido pela subcamada MAC garante que os pacotes serão transmitidos no canal sem fio pretendido e permite a troca de dados entre dispositivos com operações alternadas e concorrentes em múltiplos canais. Esta última característica permite que dispositivos WAVE com uma única camada física (ou seja, uma única antena) utilizem o acesso alternado na coordenação de canal. Como exemplo, num dispositivo WAVE de única camada física os tráfegos de alta prioridade e de gerenciamento operariam no canal de controle em intervalo de tempo 0 enquanto o tráfego geral de camadas superiores atuaria no canal de serviço durante intervalo de tempo 1. 
	
	\par Os dispositivos WAVE de várias camadas físicas podem utilizar um acesso contínuo na utilização de canal de controle ou canal de serviço, sem necessitar de alguma coordenação. Também é possível que haja acesso alternado entre os canais de serviço.

	\par Esta alternância entre o CCH e SCH depende de um mecanismo de sincronização entre os dispositivos participantes, que é garantido pelo serviço de sincronização multi-canal tratado no plano de gerenciamento. Também é definido um período de guarda no início de cada intervalo de tempo com o objetivo de compensar os efeitos de comutação de rádio e imprecisões no sincronismo, como mostrado na \autoref{fig_25}.
	...
	\par Datagramas IPv6, entretanto, têm os parâmetros canal, potência de transmissão e taxa de transferência armazenados no perfil do transmissor. Os pacotes IPv6 são recebidos pela subcamada MAC e colocados em filas de acordo com o nível de prioridade para a transmissão no SCH. Utilizando os parâmetros estabelecidos no perfil do transmissor, os pacotes são enviados por meio da camada física caso a MLME delibere acesso.

	\par O \textbf{serviço de priorização de pacotes} tratam do emprego de Qualidade de Serviço (\textit{Quality of Service} ou QoS) utilizando das técnicas EDCA (\textit{Enhanced Distributed Channel Access} ou Acesso ao Canal Distribuído Aprimorado).

	\par Quando um pacote WSMP ou IPv6 chega na subcamada MAC, tendo o roteamento do canal apropriadamente finalizado, esta deverá “enfileirar” os dados de acordo com a categoria de acesso indicada (valores entre 1 e 4).

	\par A \autoref{fig_26} ilustra as filas e o roteamento do canal. O mapeamento dos pacotes atribuindo o indicador de prioridade é tratado pelo padrão IEEE 802.11 (2012). Os Provedores de Serviço WAVE podem opcionalmente anunciar parâmetros EDCA via anúncio de serviços WAVE. Caso nenhum parâmetro EDCA seja especificado, o EDCA padrão é ativado.

	% INSERIR Figura 26 MAC (Multi-Canal)
	\begin{figure} [H]
		\centering
		\includegraphics[scale=.6]{figuras/cap3/26MAC(MultiCanal)}
		\caption{\label{fig_26}MAC (Multi-Canal)}
	\end{figure}
	
	\subsection{Serviços do Plano de Gerenciamento}
	
	\par Além dos serviços de gerenciamento MLME mencionados no padrão IEEE 802.11 (2012) para redes sem fio em geral, o padrão IEEE 1609.4 (2016) adiciona novos serviços necessários à tecnologia de redes veiculares. Os serviços oferecidos no plano de gerenciamento da subcamada MAC WAVE são:		
	
	\begin{itemize}
        \item Sincronização multi-canal
        \item Acesso ao canal
        \item Manutenção da Base de Gerenciamento de Informação (MIB)
        \item Reenderaçamento
        \item Outros Serviços IEEE 802.11
    \end{itemize}
	
	\par O \textbf{serviço de sincronização multi-canal} provido pela entidade da subcamada MAC (MLME) utiliza uma base de tempo comum que pode ser obtida localmente ou recebida via rádio para gerar uma função de sincronismo com objetivo de alinhar os intervalos de tempo entre os dispositivos WAVE comunicantes. A MLME gera quadros de anúncio de sincronismo para distribuição de informação de sincronismo e monitora os estes quadros recebidos.
	...
	\par Parâmetros como intervalo de guarda, tolerância de sincronismo e tempo máximo de troca de canal são gerenciados pelo serviço de gerenciamento de sincronização multi-canal.
	
	\par O \textbf{serviço de acesso ao canal} controla o acesso aos canais de rádio específicos em suporte às requisições recebidas da entidade de gerenciamento WAVE (WME ou WAVE \textit{Manager Entity}). Os tipos de acesso a canal, evidenciados pela \autoref{fig_24}, incluem acesso contínuo, acesso alternado e acesso imediato.
	
	%INSERIR Figura 24 Exemplos de acesso ao canal
	\begin{figure} [H]
		\centering
		\includegraphics[scale=.5]{figuras/cap3/24ExemplosDeAcessoAoCanal}
		\caption{\label{fig_24}Tipos de Acesso ao Canal}
	\end{figure}
	...
	\begin{description}
		...
		\begin{itemize}
			\item Exemplo: Um dispositivo WAVE configurado para operar em um único canal de serviço ou de controle.
		\end{itemize}
		
		\item[(B)] Acesso alternado: o acesso é alternado entre canais de serviço ou entre o canal de controle e um canal de serviço;
		\begin{itemize}
			\item Exemplo: Um dispositivo WAVE configurado para monitorar anúncios de serviços WAVE (WSAs) no canal de controle. Quando o canal de controle recebe um WSA de seu interesse, o dispositivo o alterna para o canal de serviço durante a faixa de tempo 1 para participar da aplicação de interesse, retornando ao canal de controle durante a faixa de tempo 0 para monitoramento de outros WSAs ou qualquer outra atividade. Alternativamente, um dispositivo pode ser configurado para alternar entre canais de serviço também.
		\end{itemize}
		
		\item[(C)] Acesso imeditado: o acesso é repentinamente mudado por tempo determinado ou indefinido;
		\begin{itemize}
			\item Exemplo: Um dispositivo é configurado para monitorar um canal de controle. Ao receber um WSA com uma aplicação de interesse, o dispositivo alterna imediatamente para um canal de serviço por um tempo específico até completar determinada atividade. Ao término desta atividade, o dispositivo retorna ao canal de controle.
		\end{itemize}		
	\end{description}
	
	\par O \textbf{serviço de manutenção da base de gerenciamento de informação} exercido pela MLME mantém uma MIB contendo informações de configuração e status dos canais.
	
	\par O \textbf{serviço de reendereçamento} realizado pela MLME permite alterações dinâmicas no endereço MAC a fim de suportar o \
	textbf{pseudonimato}. O pseudonimato é uma propriedade  em que identidades e padrões de comportamento de entidades não podem ser deduzidos pelo tráfego de rede, sendo apenas observáveis por partes propriamente autorizadas.

	\par A MLME permite \textbf{outros serviços} inclusos no padrão IEEE 802.11 (2012) possam chamados por cada canal independentemente.
	
	\section{\label{sec:llc}Subcamada LLC}
	
	\par Definida no padrão IEEE 1609.3 (2016), a subcamada de Controle de Enlace Lógico do modelo WAVE está imediatamente acima da subcamada MAC. A subcamada LLC do modelo WAVE é parte essencial no direcionamento dos dados para processamento nas camadas superiores. 
	...
	\par Conforme mostra a \autoref{fig_29}, o cabeçalho do protocolo WSMP-N (camada de rede do protocolo WSMP) é composto por um campo \textbf{subtipo} de 4 bits, um \textbf{indicador de campos opcionais WSMP-N} de 1 bit, um campo de \textbf{versão do WSMP} de 3 bits, e um campo de identificação para o protocolo de transporte (\textbf{TPID}) de 8 bits. Há também um campo opcional chamado de \textbf{informações extensíveis do elemento WAVE}.
	...
	\par O suporte para o campo \textbf{subtipo} com valor 0 (Protocolo de Redes Nulo) é obrigatório. Já o suporte para outros valores de subtipos são possíveis mas não foram descritos no padrão IEEE 1609.3 (2016).
	...
	\par O suporte para \textbf{TPID} 0 e 1 são obrigatórios. Os demais \textbf{TPIDs} não são especificados pelo padrão IEEE 1609.3 (2016).
	...
	\subsubsection{\label{sssec:wsa}Anúncio de Serviços WAVE}
	
	\par Um anúncio de serviço WAVE, ou WSA, é a compressão das informações de um serviço na forma de mensagens. Conforme a \autoref{fig_33}, um WSA formatada - denominada pelo padrão IEEE 1609.2 (2016) de \textit{Ieee1609Dot2Data} - é inserida no campo \textbf{dados WSM} e enviada pelo dispositivo WAVE com o valor do \textbf{PSID} 0x87, atribuído para anúncios de serviços WAVE.
	
	\par Esta mensagem formatada poderá ser um WSA seguro ou não seguro.
		
	%INSERIR Figura 33 Modelos de uma WSA
	\begin{figure} [H]
		\centering
		\includegraphics[scale=.4]{figuras/cap3/33ModelosWSA}
		\caption{\label{fig_33}Modelos de um WSA}
	\end{figure}
	
	\par Não há nenhuma especificação para WSAs não seguros, enquanto WSA seguros podem atender a duas especificações diferentes: mensagens assinadas ou mensagens encriptadas.
	
	\par O padrão IEEE 1609.11 (2010) propõe uma aplicação de coleta eletrônica de taxas, onde as trocas de informações para o pagamento de pedágios devem ser encriptadas com intuito de garantir a privacidade dos dados do cartão do motorista. Já mensagens assinadas são utilizadas para garantir que determinado serviço anunciado pertença a um provedor confiável. Conforme proposto por Whyte et al. (2015) todo anúncio de serviços WAVE deveria ser assinado, garantindo que punições sejam aplicadas aos provedores que divulgam WSAs maliciosos.
	...
	\par É possível configurar os dispositivos WAVE para receberem apenas anúncios de serviços WAVE assinados, de forma similar ao que acontece com a escolha de \textbf{PSIDs} aceitos pelo dispositivo.
	
	%INSERIR Figura 34 Cabeçalho WSA
	\begin{figure} [H]
		\centering
		\includegraphics[scale=.3]{figuras/cap3/34CabecalhoWSA}
		\caption{\label{fig_34}Cabeçalho WSA}
	\end{figure}
	
	\par A \autoref{fig_34} exibe o cabeçalho de uma anúncio de serviços WAVE. Pode-se perceber a existência de diversos campos, entretanto, não é de escopo deste trabalho entrar em detalhes nos informações que compõe um WSA.
	
	\subsubsection{Transmissão e recepção de mensagens curtas WAVE}
	\label{subsubsec:TransRecWSM}

	\par Visando uma melhor compreensão da transmissão e recepção de uma mensagem curta WAVE, o padrão IEEE 1609.3 (2016) propõe o seguinte exemplo de uma troca de WSMs. Uma aplicação de origem prepara os dados da mensagem para transmissão e seleciona um endereço MAC de broadcast. Baseado nas configurações especificadas para esta mensagem, a aplicação informa as configurações da camada física (potência de transmissão, taxa de transferência) e invoca a primitiva de requisição de mensagem curta WAVE (WSM-WaveShortMessage.request) para solicitar que o protocolo WSMP entregue o pacote resultante para as camadas inferiores com o intuito de serem transmitidas no canal selecionado.
	...
	\par Na camada de rede, o padrão IEEE 1609.3 (2016) oferece suporte ao protocolo IPv6 obedecendo as especificações da norma IETF RFC 2460. O tráfego IP das Redes Veiculares é transmitido e recebido por meio da subcamada de enlace LLC com o campo \textit{Type} contendo valor \textit{Ethertype} 0x86DD. 
	...
	\par As BSMs são mensagens para aplicações de segurança em veículos. Estas aplicações utilizariam o fluxo de dados de BSMs provenientes de veículos nas redondezas para detectar potenciais eventos de risco. As BSMs fornecem aos veículos conhecimento da posição, velocidade e direção dos veículos nas imediações. Estas mensagens de segurança possuem grupos de PSIDs para dados específicos. Conforme mostrado na \autoref{tab_9} (\autoref{psid_aloc}), para BSMs contendo dados de alta precisão o grupo PSID utilizado é o 0x20. Para veículos com dados não tão acurados, o grupo de PSID utilizado é o 0x21. 
	...
	\par Baseando-se no padrões IEEE 1609.3, IEEE 1609.4, ISO 14906 e ISO 15628, o padrão IEEE 1609.11 (2010) propõe um sistema de pagamento eletrônico para redes veiculares foi proposto como aplicação em redes veiculares. As normas citadas estabelecem as diretrizes para alcance de confiabilidade e segurança no pagamento. 
	...
	\begin{figure} [H]
		\centering
		\includegraphics[scale=.45]{figuras/cap3/39AplicacaoPedagio}
		\caption{\label{fig_39}Aplicação de pedágio}
	\end{figure}
	
	\par A antena no CCH também envia anúncio de serviços WAVE (WSA) que divulgam a presença da praça de pedágio no percurso e entregam os parâmetros do serviço como o canal, método de acesso ao canal, PSID e demais parâmetros.
	
	\par Quando uma unidade móvel (OBU) recebe o WSA de um Serviço de Coleta Eletrônica de Taxas, a entidade de gerenciamento WAVE (WME) troca para o canal de serviço (SCH) e método de acesso descritos pelo WSA, que no exemplo é respectivamente SCH 182 com acesso contínuo. Informações da aplicação na \autoref{tab_4}.
	...
	\begin{table}[H]
		...
		\begin{tabular}{|c|c|p{6cm}|} \hline
			...
			Tipo WSA & não seguro & WSA não assinado \\ \hline
			...
			Taxa de repetição & 250 &	WSA transmitido 5 vezes a cada 100ms \\ \hline
...
%-------------------------------------------------------------
	\newpage
	\chapter{Abordagens Avaliativas}
	\label{chap:Abor_Aval}
	
	\par Toda e qualquer tecnologia precisa ser testada utilizando-se de alguma abordagem de avaliação antes de entrar em caráter de produção no mercado. Abordagens avaliativas são métodos distintos de pensar, desenhar e conduzir atividades de avaliação. A utilização de abordagens avaliativas é de considerável importância para o método científico. Quando uma abordagem é corretamente empregada em determinada tecnologia, esta se torna fundamental para antecipar possíveis inconvenientes ou conflitos existentes no recurso tecnológico antes do mesmo entrar em produção.
	...
	\par Confome corroborado também por Olariu e Weigle (2009), a maneira mais apropriada de se avaliar o desempenho de uma rede é desenvolvendo simulações que forneçam resultados mais próximos de observações do mundo real.
	...
	\par Seguindo o critério estabelecido acima foi visto que há diversos modelos de microtráfego urbano implementados. Segundo conclusão de Brockfeld e Wagner (2004), todos os modelos microscópicos de simulação apresentam um mesmo valor de acurácia como modelos de mobilidade. Isto inclui os modelos \textit{Cellular Automaton}, \textit{Car-following} e IDM/MOBIL.
	...
	\par Segundo Krajzewicz (2012), o SUMO faz a modelagem de tráfego microscópica, ou seja, cada veículo tem sua própria rota e se move individualmente na rede. As simulações são determinísticas por padrão, contudo há um conjunto de configurações que introduzem aleatoriedade no software.
	...
	\par Conforme descrito na enciclopédia on-line do SUMO{\footnote{\href{http://sumo.dlr.de/wiki/Simulation/Randomness}{http://sumo.dlr.de/wiki/Simulation/Randomness} (acesso em 13 de out. 2016)}}, estas sementes aleatórias são produzidas por um gerador de números pseudoaleatórios chamado \textit{Mersenne Twister}, garantindo efeitos mais aproximados de cenários reais.

	\par As simulações realizadas nos cenários subsequentes utilizam o mesmo espectro descrito no IEEE 1609.0 (2013) que foi regulamentado pela FCC para comunicações DSRC nos Estados Unidos. A ANATEL (2012) ainda não estabeleceu um espectro para as comunicações interveiculares no Brasil até a presente data.
    ...
    \begin{table}[H] %[?] ARRUMAR tamanho da tabela
    	\centering
	    ...
	    \begin{tabular}{|p{4.8cm}|p{10.2cm}|}
    		...
    		ApplicationManager & Classe gerente de aplicações, inspirado em Gamma et al. (2007) no Padrão de Projeto Composite. Permite ao dispositivo WAVE executar diversas aplicações ao mesmo tempo, diferente dos exemplos do VEINS, onde cada dispositivo executa exclusivamente uma aplicação. \textit{BaseApplicationMsg} é uma classe abstrata que contém informações comuns à aplicações WAVE. É possível a adição de novas classes no futuro como aplicações de clima, comboio de veículos, segurança no tráfego, etc. \\ \hline
	...
	\par Devido à sua natureza, a aplicação de emergência utiliza o PSID \textbf{0x0C} no cabeçalho das mensagens, apropriado para situações de emergência. O código abaixo representa como essa informação é gerada pelo veículo de segurança pública via função \textbf{prepareMessage}.

    \begin{table}[H] %[?] Colocar cor no codigo.
	    \centering
	...
	\begin{table}[H]
		...
		\begin{tabular}{|l|c|c|}
			...
			\textbf{Total percorrido} & 4000 m & 4000 m \\ \hline
			\textbf{Tempo total gasto} & 128,62 s & 120,82 s \\ \hline
			\textbf{Velocidade média} & 111,95 km/h & 119,18 km/h \\ \hline
		\end{tabular}
	...
	\begin{table}[H]
		...
		\begin{tabular}{|l|c|p{5.2cm}|}
			...
			\textbf{Total percorrido} & 4000 m & \multicolumn{1}{c|}{4000 m} \\ \hline
			\textbf{Tempo total gasto} & 128,62 s & \multicolumn{1}{c|}{120,68 s} \\ \hline
			\textbf{Velocidade média} & 111,95 km/h & \multicolumn{1}{c|}{119,20 km/h} \\ \hline
		\end{tabular}
	...
	\subsection{Experimento 3: Aplicação de Emergência em região existente}
	
	\par Para o terceiro experimento, um cenário mais complexo foi desenvolvido para a realização das simulações. Foi importado o mapa do entorno da região de Ipanema-Copabana na cidade do Rio de Janeiro por meio do site \href{https://www.openstreetmap.org}{openstreetmap.org/}. Esta região foi escolhida por serem dois bairros turísticos mundialmente famosos da cidade do Rio de Janeiro{\footnote{\href{http://www.touropia.com/tourist-attractions-in-rio-de-janeiro/}{http://www.touropia.com/tourist-attractions-in-rio-de-janeiro/} (acesso em 20 oct. 2016)}},  além do fato deles também possuírem condições propícias à simulações de tráfego, como melhor planejamento urbano, número relevante de faixas por vias, semáforos, obstáculos, etc.
	...
	\par Os dados coletados no experimento 3 tiveram seus \textit{outliers} filtrados. Um \textit{outlier} é descrito por Hawkins (1980) como um ponto que está muito distante das demais observações em uma série. Comumente é chamado de “ponto fora da curva”. A utilização da análise de \textit{outliers} otimiza análises e garante confiabilidade na amostra.
	...
	\begin{table}[H]
		...
		\begin{tabular}{|l|c|}
			\hline
			\multicolumn{1}{|c|}{\textbf{Métricas}} & \textbf{Medição base} \\ \hline
			\textbf{Total percorrido} & 4800 m \\ \hline
			\textbf{Média de tempo total} & 477,11 s \\ \hline
			\textbf{Média após Análise de \textit{Outliers}} & 471,53 s \\ \hline
			\textbf{Velocidade média} & 36,65 km/h \\ \hline
		\end{tabular}
	...
	\begin{table}[H]
		...
		\begin{tabular}{|l|c|}
			\hline
			\multicolumn{1}{|c|}{\textbf{Métricas}} & \textbf{Aplicação de emergência com RSUs} \\ \hline
			\textbf{Total percorrido} & 4800 m \\ \hline
			\textbf{Média de tempo total} & 458,06 s \\ \hline
			\textbf{Média após Análise de \textit{Outliers}} & 452,25 s \\ \hline
			\textbf{Velocidade média} & 38,21 km/h \\ \hline
		\end{tabular}
	...
	\begin{table}[H]
		...
		\begin{tabular}{|l|p{7.5cm}|}
			\hline
			\multicolumn{1}{|c|}{\textbf{Métricas}} & \textbf{Aplicação de emergência com RSUs} (semáforos abertos) \\ \hline
			\textbf{Total percorrido} & \multicolumn{1}{c|}{4800 m} \\ \hline
			\textbf{Média de tempo total} & \multicolumn{1}{c|}{304,12 s} \\ \hline
			\textbf{Média após Análise de \textit{Outliers}} & \multicolumn{1}{c|}{303,53 s} \\ \hline
			\textbf{Velocidade média} & \multicolumn{1}{c|}{56,93 km/h} \\ \hline
		\end{tabular}
	...
	\par Podemos perceber uma melhora marginal nos tempos totais de trajeto do veículo de resgate em todos os cenários. Esta melhora marginal (valores entre 4,09 e 6,17\%) pode ser atribuída exclusivamente ao uso da aplicação de emergência desenvolvida para o estudo de caso. As estatísticas foram limitadas no seu escopo à características do software de simulação de tráfego que, se fossem minimizadas, acarretariam em números ainda mais significativos.
	
	\par As limitações percebidas no software de simulação de tráfego SUMO foram enumeradas:
	...
	\begin{table}[H]
		...
		\begin{tabular}{|p{3.5cm}|p{3.0cm}|p{3.1cm}|p{4.0cm}|}
			...
			Experimento 3 & 471,53 s & 452,25 s & 4,09\% \\ \hline
...
% ----------------------------------------------------------
	...
	\par Algumas limitações foram notadas e listadas quanto ao software de simulação de tráfego urbano. Estas limitações nos requisitos do software cercearam as estatísticas coletadas no estudo de caso a uma melhora marginal no tempo total de trajeto do veículo de segurança pública. Ainda assim, o estudo de caso revelou em todos os experimentos de simulação que o tempo total de trajeto de veículos de segurança pública foi reduzido devido a utilização de aplicação de emergência empregada em redes veiculares.
...
% ----------------------------------------------------------
	...
	\par Estudos na manipulação física do envio de mensagens WAVE podem auxiliar na diminui-\\ção de congestionamento de sinal em rodovias movimentadas. É possível diminuir ainda mais essa inundação controlando a replicação das mensagens, seja por meio da limitação do tempo de vida dos pacotes ou com a utilização de quadros de gerenciamento para o envio \textit{multicast}.

	\par Provas de conceito utilizando de idéias introduzidas no código da aplicação de emergência poderiam ser testadas com o porte de equipamentos de redes veiculares desenvolvidos pelo mercado até o presente momento. O que poderia corroborar ou até mesmo aperfeiçoar os requisitos e métricas da aplicação de emergência.

	\par A partir de novas pesquisas realizadas no Brasil, as agências reguladoras e empresas de tecnologia nacionais voltariam sua atenção ao tema proposto e poderiam estabelecer marcos regulatórios e estratégicos para a difusão das redes veiculares no mercado nacional.
...
% ----------------------------------------------------------
	...
	\begin{itemize}[label={}] %\itemsep -4pt
		...
		\item TANENBAUM, Andrew S.; WETHERALL, DAVID J.; Computer Networks. Fifth Edition. Pearson Education, 2011. 960p.
        ...
        \item SAVAGE, I. Comparing the fatality risks in United States transportation across modes and over time. Research in Transportation Economics, v. 43, n. 1, p. 9-22, 2013.
		...
		\item IEEE COMPUTER SOCIETY. Wireless Access in Vehicular Enviroments: Amendment 6, 802.11p-2010. Nova York, 2010. 51 p.
		...
		\item BROCKFELD, E.;  WAGNER, P. Testing and Benchmarking of Microscopic Traffic Flow Simulation Models; 10th World Conference on Transport Research; July 2004.
		...
		\item GAMMA, E., et al. Padrões de Projeto: Soluções reutilizáveis de software orientado a objetos. Tradução de Luiz A. Meirelles Salgado. Bookman, 2007. 368 p.

		\item SOMMER, C.; DRESSLER, F. Vehicular Networking. Primeira Edição. Cambridge University Press, 2015. 370 p.

		\item CET-RIO. Volume Diário De Veículos Das Principais Vias Do Município Do Rio De Janeiro. Disponível em: \url{http://www.rio.rj.gov.br/dlstatic/10112/5112752/4131653/VolumedasprinciaisviasdoRiodeJaneiro.pdf}. Acesso em: 25 de ago. 2016.

        \item HAWKINS, D. M. Identification of Outliers. Chapman and Hall, 1980. 188 p. (Monographs on applied probability and statistics)
	...
	\par O próximo capítulo irá abordar brevemente a história das Redes Veiculares, apresentar a família de padrões WAVE e explicar em detalhes a pilha de protocolos WAVE.
		...
		\chapter{\label{psid_aloc}Alocação de PSIDs}
		
		\par Na família de padrões WAVE são alocados identificadores a fim de identificar dispositivos, serviços, organizações e tipos de mensagens WAVE segundo seus propósitos. A norma que aloca os identificadores de dispositivos ou serviços WAVE é a IEEE 1609.12 (2016).
		...
		\begin{itemize}
			...
			\item Conforme o padrão WAVE de Serviços de Rede (IEEE 1609.3), um provedor de serviço identifica os anúncios de serviços WAVE pelos valores PSID contidos nas mensagens WSM transmitidas.
	    ...
	    \par Partes do cabeçalho WSMP e WSA permitem a presença do campo Informações Extensíveis do Elemento WAVE que deverá conter a estrutura apresentada na \autoref{fig_35} conforme descrito no padrão IEEE 1609.3 (2016).
		...
		\chapter{\label{tutorial_1}Preparando o ambiente para simulações veiculares no VEINS}
		    ...
		    \par O download do SUMO 0.25.0 pode ser feito por meio do site \href{https://sourceforge.net/projects/sumo/files/sumo/}{https://sourceforge.net/\\projects/sumo/files/sumo/}, escolhendo a versão compatível com o sistema operacional desejado.

		    \par O download do OMNET++ 4.6 pode ser feito por meio do site \href{https://omnetpp.org/omnetpp}{https://omnetpp.org/\\omnetpp}, escolhendo a versão compatível com o sistema operacional desejado.

		    \par O download do VEINS 4.4 pode ser feito no site \href{http://veins.car2x.org/download/}{http://veins.car2x.org/\\download/}.
        	...
        	\section{Testar funcionamento do SUMO}
	            ...
	            \end{description}
	            \par Abra o exemplo test.sumo.cfg dentro da pasta do SUMO no caminho .../sumo-0.25.0/\\tools/contributed/traci4j/test/resources/sumo\_maps/box1l por meio do menu \textit{File > Open Simulation}.
		...
		\chapter{\label{tutorial_2}Desenvolvendo Simulações}
        	...
        	\section{Gerando rotas aleatórias}
            	...
            	\begin{description}
                    \item[11º Passo]
                \end{description}
                \par No diretório \textbf{simulacao} (1º Passo do \autoref{tutorial_2}), adapte a linha de comando abaixo e execute-a no terminal: %[!] referenciar
        	...
        	\section{Simulando a Rede Veicular}
            	...
            	\begin{description}
                    \item[17º Passo]
                \end{description}
                \par Copie os arquivos \textbf{omnetpp.ini}, \textbf{RSUExampleScenario.ned} e \textbf{erlangen.laun-\\chd.xml} localizados no diretório \textit{veins-veins-4.4 > examples > \textbf{veins}} para a pasta \textit{veins-veins-4.4 > examples > \textbf{simulacao}}.
